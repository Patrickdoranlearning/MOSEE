  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (result?.error) {
        ...
        window.location.href = '/wealth-tree/onboarding'
      }
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Plant Your Wealth Tree</h1>
          <p className="mt-2 text-gray-600">
            Begin your journey to financial wisdom
          </p>
        </div>

        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
          {/* Google OAuth — only shown after mount and when configured */}
          {mounted && googleEnabled && (
            <>
              <button
                onClick={() => signIn('google', { callbackUrl: '/wealth-tree/onboarding' })}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 px-4 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
